...
async def file_html(request):
    ...
    download_link = f"https://ddlserverv1.me.in/dl/{hash}"
...
async def main(client, message):
    ...
    dl_markup = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(text="🔗 Download Link", url=f"https://ddlserverv1.me.in/dl/{hash}")
            ]
        ]
    )
    ...
    send = await message.reply_text(f"**File Name:** `{filenam}`\n\n**Download Link:** `https://ddlserverv1.me.in/dl/{hash}`", reply_markup=dl_markup)
    msg_id=int(taku.id)
    save_file_in_db(filename, filenam, hash, msg_id)
...
async def rename_doc(bot, update):
    if (" " in update.text) and (update.reply_to_message is not None):
        ...
        hax = jar.text.split("\n\n")[1].split(": ")[1]
        print(hax)
        linkx = hax.replace("https://ddlserverv1.me.in/dl/", "")
        ...
        if idx:
            ...
            await goat.edit_message_text(
                chat_id=user_id,
                message_id=repl,
                text=f"**File Name**: `{file_name}`\n\n**Download Link:** `https://ddlserverv1.me.in/dl/{linkx}`", 
